...
import os, joblib, re, requests, json, cv2
import numpy as np 
import pandas as pd
import matplotlib.pyplot as plt
import re, urllib3, base64
...
@aclsf_bp.route('/detection', methods=['GET', 'POST'])
def detection():
    if request.method == 'GET':
        return render_template('advanced/detection.html', menu=menu, weather=get_weather())
    else:
        f_img = request.files['image']
        file_img = os.path.join(current_app.root_path, 'static/upload/') + f_img.filename
        f_img.save(file_img)
        ...
        _, image_type = os.path.splitext(file_img)
        ...
        drawn_img_file = os.path.join(current_app.root_path, 'static/img/drawn_res/') + 'drawn_' + f_img.filename
        image.save(drawn_img_file)

        mtime = int(os.stat(file_img).st_mtime)
        return render_template('advanced/detection_res.html', menu=menu, weather=get_weather(), mtime=mtime,
                               org_file=f_img.filename, new_file='drawn_'+f_img.filename)